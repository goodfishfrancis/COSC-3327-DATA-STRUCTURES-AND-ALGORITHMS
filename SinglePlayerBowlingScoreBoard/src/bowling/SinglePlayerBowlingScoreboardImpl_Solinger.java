...
//		assert (boxIndex != 2) || (!isSpare(frameNumber)) : "boxIndex = " + boxIndex + ", but there was a Spare! : frameNumber = " + frameNumber;
	...
	public int getScore(int frameNumber) 
	{
		...
		//Map<Integer, Integer> frameToScoreMap = getFrameToScoreMap();
		
		//return frameToScoreMap.get(frameNumber);
	...
	public boolean isGameOver() 
	{
		...
		//return ((18 - strikeCount) == rollCount);
		return (rollCount == 18);
	}
	...
	/*private Map<Integer, Integer> getFrameToScoreMap()
	{
		...
		for (int i = 1; i < 10; i++)
		{
			box1_pinsKnockedDown = getMarkToPinsKnockedDownMap().get(getFrameToBoxToMarkMapMap().get(i).get(1));
			box2_pinsKnockedDown = getMarkToPinsKnockedDownMap().get(getFrameToBoxToMarkMapMap().get(i).get(2));
			score += (box1_pinsKnockedDown + box2_pinsKnockedDown);
			
			frameToScoreMap.put(i, score);
